...
const ElectronicRepair = (props) => {
  const Data = [];

  const getData = () => {
    props.props.map((user) => {
      if (user.category === "electronicRepair") {
        Data.push(user);
      }
    });
  };
  ...
  return (
    <BrowserRouter>
      <Route
        exact
        path="/electronicRepair"
        name="ElectronicRepair"
        render={() => (
          <div className={classes.wrapper}>
            <h2 className={classes.title}>РЕМОНТ ТЕХНИКИ</h2>
            ...
            <div className={classes.categoryWrapper}>
              {Data.map((user) => (
                <div className={classes.profileWrap} key={user.id}>
                  <Link to={"/ElectronicRepair/" + user.id}>
                    <Profile props={user} />
                  </Link>
                </div>
              ))}
      ...
      />
      <Route exact path="/ElectronicRepair/:id">
...
export default withRouter(ElectronicRepair);
